      ...
      try {
              ...
              if (first.type === "think_start") {
                ...
                if (index !== undefined) {
                  ...
                  if (cleanedArg.startsWith("```json")) {
                    cleanedArg = cleanedArg.slice(7);
                  } else if (cleanedArg.startsWith("```")) {
                    cleanedArg = cleanedArg.slice(3);
                  }
                  if (cleanedArg.endsWith("```")) {
                    cleanedArg = cleanedArg.slice(0, -3);
                  }
        ...
        const processLine = (line: string) => {
          ...
          try {
            ...
            console.log(
              `[QwenCNWebStream] Debug data.data: ${JSON.stringify(data.data)?.slice(0, 200)}`,
            );
            console.log(
              `[QwenCNWebStream] Debug data.communication: ${JSON.stringify(data.communication)?.slice(0, 200)}`,
            );
