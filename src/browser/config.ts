...
function ensureDefaultChromeExtensionProfile(
  ...
  result.chrome = {
    // Keep the persisted profile schema on the canonical config driver values;
    // `resolveProfile()` maps this built-in relay profile back to extension mode.
    driver: "clawd",
...
export function resolveBrowserConfig(
  ...
  const controlPort = gatewayPort + BROWSER_CONTROL_PORT_OFFSET;
  const defaultColor = normalizeHexColor(cfg?.color);
  const remoteCdpTimeoutMs = 1500;
  const remoteCdpHandshakeTimeoutMs = 3000;
...
export function resolveProfile(
  ...
  let cdpUrl: string;
  const driver = profileName === "chrome" && profile.driver === "clawd" ? "extension" : "openclaw";
