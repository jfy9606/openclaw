  ...
  return (model, context, streamOptions) => {
    ...
    const run = async () => {
      try {
        ...
        console.log(
          `[DeepseekWebStream] Context messages count: ${messages.length}, hasSystemPrompt: ${Boolean(systemPrompt)}`,
        );
        let prompt = "";
        ...
        const searchEnabled =
          (streamOptions as unknown as { searchEnabled?: boolean })?.searchEnabled ?? true;
        const preempt = (streamOptions as unknown as { preempt?: boolean })?.preempt ?? false;
        const fileIds = (streamOptions as unknown as { fileIds?: string[] })?.fileIds || [];
