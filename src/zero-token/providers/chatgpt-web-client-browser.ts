...
// DOM globals below run inside Playwright page.evaluate callbacks (browser page context).
// Core tsconfig targets Node and deliberately omits the DOM lib, so declare them locally.
...
type HTMLInputElement = DomElement;
type HTMLTextAreaElement = DomElement;
type HTMLButtonElement = DomElement;
...
export class ChatGPTWebClientBrowser {
  ...
  private async ensureBrowser() {
    ...
    const rootConfig = getRuntimeConfig();
    ...
    try {
      await this.page.waitForFunction(
        () => {
          ...
          return scripts.some(
            (s: { src: string }) => s.src?.includes("oaistatic.com") && s.src?.endsWith(".js"),
          );
        },
        { timeout: 15000 },
      );
  ...
  private async chatCompletionsViaDOM(params: {
    ...
    for (let elapsed = 0; elapsed < maxWaitMs; elapsed += pollIntervalMs) {
      ...
      const result = await page.evaluate(() => {
        ...
        const isStreaming = Boolean(stopBtn);
    ...
    const responseData = await page.evaluate(
      async ({ body: reqBody, pageUrl: reqPageUrl }) => {
        ...
        async function getSession() {
          ...
          return r.ok
            ? (r.json() as Promise<{ accessToken?: string; oaiDeviceId?: string }>)
            : null;
        }
