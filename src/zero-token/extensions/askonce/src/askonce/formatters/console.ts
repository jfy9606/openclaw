...
export class ConsoleFormatter {
  format(result: QueryResult): string {
    ...
    // 按响应时间排序
    const sortedResponses = [...result.responses].toSorted(
  ...
  private formatResponse(response: ModelResponse, _index: number): string {
    const lines: string[] = [];
  ...
  private formatSummary(result: QueryResult): string {
    ...
    if (completed.length > 0) {
      lines.push(chalk.bold("\n📏 回答长度:"));
      completed
        .toSorted((a, b) => b.charCount - a.charCount)
        .forEach((r) => {
