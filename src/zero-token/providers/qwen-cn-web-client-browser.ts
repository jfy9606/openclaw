...
export class QwenCNWebClientBrowser {
  ...
  private async ensureBrowser() {
    ...
    if (browserConfig.attachOnly) {
      ...
      if (!browser) {
        throw new Error("Failed to connect to a browser context for Qwen CN");
      }
      ...
      const browser = this.browser;
      if (!browser) {
        throw new Error("Failed to create a browser context for Qwen CN");
      }
    ...
    if (cookies.length > 0) {
      try {
        const browser = this.browser;
        if (!browser) {
          throw new Error("Browser context missing before cookie injection");
        }
  ...
  async chatCompletions(params: {
    ...
    const responseData = await page.evaluate(
      async ({
        ...
        try {
          const url = `${baseUrl}/api/v2/chat?biz_id=ai_qwen&chat_client=h5&device=pc&fr=pc&pr=qwen&nonce=${reqNonce}&timestamp=${reqTimestamp}&ut=${ut}`;
    ...
    if (responseData?.data && responseData.data.length > 0) {
      console.log(`[Qwen CN Web Browser] Response preview: ${responseData.data.slice(0, 200)}...`);
    }
