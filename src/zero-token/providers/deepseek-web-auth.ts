...
// Page-context DOM globals used inside the browser evaluate callback below.
  ...
  try {
    ...
    if (
      ...
      try {
        ...
        for (const [key, value] of Object.entries(pageStorage)) {
      ...
      (resolve, reject) => {
        ...
        const tryResolve = async () => {
          ...
          try {
            ...
            const parsedCookieString = cookies.map((c) => `${c.name}=${c.value}`).join("; ");

            // d_id is preferred, but ds_session_id is an extremely strong signal of an active session
            const parsedHasDeviceId = parsedCookieString.includes("d_id=");
            const parsedHasSessionId = parsedCookieString.includes("ds_session_id=");
            const parsedHasSessionInfo =
              parsedCookieString.includes("HWSID=") || parsedCookieString.includes("uuid=");

            if (parsedHasDeviceId || parsedHasSessionId || parsedHasSessionInfo || cookies.length > 3) {
              resolved = true;
              clearTimeout(timeout);
              if (checkInterval) {
                clearInterval(checkInterval);
