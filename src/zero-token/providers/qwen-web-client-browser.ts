...
export class QwenWebClientBrowser {
  ...
  async chatCompletions(params: {
    ...
    console.log(`[Qwen Web Browser] Message: ${params.message.slice(0, 100)}...`);
      ...
      async ({ baseUrl, timeoutMs }) => {
        ...
        try {
          ...
          if (!res.ok) {
            ...
            console.log(`[Browser] Create chat error response: ${errorText.slice(0, 500)}`);
    ...
    const responseData = await page.evaluate(
      async ({ baseUrl, chatId: reqChatId, model: reqModel, message, fid: reqFid, timeoutMs }) => {
        let timer: ReturnType<typeof setTimeout> | undefined = undefined;
        try {
          const url = `${baseUrl}/api/v2/chat/completions?chat_id=${reqChatId}`;
          ...
          if (!res.ok) {
            const errorText = await res.text();
            console.log(`[Browser] Error response: ${errorText.slice(0, 500)}`);
            return { ok: false, status: res.status, error: errorText };
          }
          ...
          while (true) {
            ...
            chunkCount++;
            if (chunkCount <= 3) {
              console.log(`[Browser] Chunk ${chunkCount}: ${chunk.slice(0, 200)}`);
            }
          }
    ...
    console.log(
      `[Qwen Web Browser] Response preview: ${responseData.data?.slice(0, 300) || "empty"}`,
    );
