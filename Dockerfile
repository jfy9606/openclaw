...
RUN echo '# Homebrew environment setup' > /etc/profile.d/homebrew.sh && \
  echo 'export HOMEBREW_PREFIX="/home/linuxbrew/.linuxbrew"' >> /etc/profile.d/homebrew.sh && \
...
# Add Conda to profile.d so login shells have correct PATH
RUN echo '# Conda environment setup' > /etc/profile.d/conda.sh && \
  echo 'export CONDA_HOME="/opt/conda"' >> /etc/profile.d/conda.sh && \
  echo 'export CONDA_DEFAULT_ENV="base"' >> /etc/profile.d/conda.sh && \
  echo 'export CONDA_PREFIX="/opt/conda"' >> /etc/profile.d/conda.sh && \
  echo 'export CONDA_ENVS_DIRS="/home/node/.openclaw/workspace/conda_envs"' >> /etc/profile.d/conda.sh && \
  echo 'export CONDA_PKGS_DIRS="/home/node/.openclaw/workspace/conda_pkgs"' >> /etc/profile.d/conda.sh && \
  echo 'export PATH="/opt/conda/bin:$PATH"' >> /etc/profile.d/conda.sh && \
  echo 'if [ -f "/opt/conda/etc/profile.d/conda.sh" ]; then . "/opt/conda/etc/profile.d/conda.sh"; fi' >> /etc/profile.d/conda.sh && \
...
# ==================================================
# Copy Homebrew from build stage (already installed with go)
# ==================================================
COPY --from=build --chown=node:node /home/linuxbrew /home/linuxbrew
...
# ==================================================
# Copy Miniconda from build stage (already installed with openclaw env)
# ==================================================
COPY --from=build --chown=node:node /opt/conda /opt/conda
...
# Redirect conda envs and pkgs to workspace (persistent storage)
RUN rm -rf /opt/conda/envs /opt/conda/pkgs && \
  ln -s /home/node/.openclaw/workspace/conda_envs /opt/conda/envs && \
  ln -s /home/node/.openclaw/workspace/conda_pkgs /opt/conda/pkgs
