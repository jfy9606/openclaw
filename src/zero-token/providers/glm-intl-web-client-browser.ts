...
type HTMLElement = { textContent: string | null };
declare const document: { querySelectorAll(selector: string): HTMLElement[] };
...
function generateSign(): { timestamp: string; nonce: string; sign: string } {
  ...
  const a = i % 10;
  const timestamp = A.slice(0, t - 2) + a + A.slice(t - 1, t);
        ...
        try {
          const res = await fetch("https://chat.z.ai/chatglm/user-api/user/refresh", {
            ...
            headers: {
              ...
              Authorization: `Bearer ${reqRefreshToken}`,
              "App-Name": "chatglm",
              "X-App-Platform": "pc",
              "X-App-Version": "0.0.1",
              "X-Device-Id": deviceId,
              "X-Request-Id": reqRequestId,
              "X-Sign": reqSign.sign,
              "X-Nonce": reqSign.nonce,
              "X-Timestamp": reqSign.timestamp,
            },
            credentials: "include",
            body: JSON.stringify({}),
          });
          ...
          if (!accessToken) {
            return {
              ok: false,
              status: 200,
              error: `No accessToken in response: ${JSON.stringify(data).slice(0, 300)}`,
            };
          }
