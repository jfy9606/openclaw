...
export class ClaudeWebClientBrowser {
  ...
  async createConversation(): Promise<ClaudeConversation> {
    ...
    const response = await page.evaluate(
      async ({ url: reqUrl, deviceId, convUuid: reqConvUuid }) => {
  ...
  async chatCompletions(params: {
    ...
    const responseData = await page.evaluate(
      async ({ url: reqUrl, body: reqBody, deviceId }) => {
        const res = await fetch(reqUrl, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
    ...
    console.log(
      `[Claude Web Browser] Response preview: ${responseData.data?.slice(0, 200) || "empty"}`,
    );
