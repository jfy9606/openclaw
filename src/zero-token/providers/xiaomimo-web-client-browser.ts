...
declare const XMLHttpRequest: new () => {
...
export class XiaomiMimoWebClientBrowser {
  ...
  private async ensureBrowser() {
    ...
    const mimoPage = pages.find((p) => p.url().includes("xiaomimimo.com"));
    if (mimoPage) {
      this.page = mimoPage;
    } else {
  ...
  async chatCompletions(params: {
    ...
    const result = await page.evaluate(
      async ({
        message: reqMessage,
        modelInternal: reqModelInternal,
        convId: reqConvId,
        msgId: reqMsgId,
      }: {
        message: string;
        ...
        console.log("[MiMo XHR] Body:", JSON.stringify(body).slice(0, 200));
